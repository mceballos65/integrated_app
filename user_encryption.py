...
def save_github_username(username: str) -> bool:
    """Save GitHub username in encrypted storage"""
    try:
        data = decrypt_data()
        if "github_token" not in data:
            data["github_token"] = {}
        ...
        data["github_token"]["username"] = encrypted_username
        data["github_token"]["updated_at"] = datetime.now().isoformat()
        
        save_users_data(data)
        print("GitHub username saved securely")
        return True
    except Exception as e:
...
def save_github_credentials(token: str, username: str) -> bool:
    """Save both GitHub token and username in encrypted storage"""
    try:
        data = decrypt_data()
        if "github_token" not in data:
            data["github_token"] = {}
        
        # Encrypt both the token and username before storing
        encrypted_token = encrypt_string(token)
        encrypted_username = encrypt_string(username)
        ...
        save_users_data(data)
        print("GitHub credentials saved securely")
        return True
    except Exception as e:
